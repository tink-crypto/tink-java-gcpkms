...
if [[ "${IS_KOKORO}" == "true" ]] ; then
  # Note: KOKORO_GIT_COMMIT_tink_java_gcpkms and
  # GITHUB_ACCESS_TOKEN are populated by Kokoro.
  GITHUB_RELEASE_UTIL_OPTS+=(
    -c "${KOKORO_GIT_COMMIT_tink_java_gcpkms}"
...
if [[ ! -f "${RELEASE_UTIL_SCRIPT}" ]]; then
  ...
  echo "Make sure you run this script from the root of tink-java-gcpkms." >&2
  exit 1
fi

pushd "${TMP_FOLDER}"
"${RELEASE_UTIL_SCRIPT}" "${GITHUB_RELEASE_UTIL_OPTS[@]}" create_tag \
  "${RELEASE_VERSION}" tink-java-gcpkms
popd
